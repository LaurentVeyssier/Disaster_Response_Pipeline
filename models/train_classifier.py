...
def load_data(database_filepath, augmentation=True):
    ...
    X = df.message #df.select_dtypes(include=['object']).drop('original',axis=1)
...
def data_augmentation(dataframe):
    ...
    under_represented_labels = sorted_labels[dataframe[labels].sum(axis=0).sort_values()<1000]

    # perform data augmentation on under-represented classes
    new_rows = []
    df_to_augment = dataframe.loc[dataframe[under_represented_labels].sum(axis=1)==1,:]
...
def build_model():
    ...
    outputs:
        - ML model
    """
    pipeline = Pipeline([   ('tokenization', CountVectorizer(tokenizer=tokenize,
                                                             min_df=3,
                                                             max_features=10000)
                                                             ),
                        ('vectorization',TfidfTransformer()),
                        ('modelisation', MultiOutputClassifier(estimator=XGBClassifier(), 
                                                                n_jobs=-1)
                                                                )
                    ])
    return pipeline
